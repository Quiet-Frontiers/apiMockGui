 ...
 */
export async function setupMswServiceWorker(publicDir: string = './public'): Promise<void> {
  try {
...
export const mswHelpers = {
   ...
   */
  loadConfigFromLocalStorage: (key: string = 'api-mock-config'): MockApi[] => {
    try {
   ...
   * localStorage에 설정 저장
   */
  saveConfigToLocalStorage: (apis: MockApi[], key: string = 'api-mock-config'): void => {
    try {
